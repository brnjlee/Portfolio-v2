...
class Scene extends Component {
  constructor(props, context) {
    ...
    this._onAnimate = () => {
      if (this.props.isMobile) {
        ...
        this.camera.position.x -=
          (this.mouseX / 30 + this.camera.position.x + 10) * 0.05;
        this.camera.position.y -=
          -(this.mouseY / 30 - this.camera.position.y) * 0.05;
      }
      this.camera.lookAt(this.scene.position);

      for (let i = 0; i < this.particleCount; i++) {
        if (
          this.particleRefs[i].direction.y > 0 &&
          i < this.particleCount / 2
        ) {
        ...
        if (
          this.particleRefs[i].direction.x > 0 &&
          i < this.particleCount / 2
        ) {
    ...
    let particleGeometry = (
      <sphereGeometry radius={20} widthSegments={20} heightSegments={20} />
    );
    let particleColors = [0xffffff];
    ...
    for (let i = 0; i < this.particleCount; i++) {
      this.particles[i] = (
        <mesh
          key={i}
          ref={particle => {
            this.particleRefs.push(particle);
  ...
  handleScroll = e => {
    ...
    this.camera.rotation.set(
      this.camera.rotation.x + e.deltaY * 0.005,
      this.camera.rotation.y + e.deltaY * 0.005,
